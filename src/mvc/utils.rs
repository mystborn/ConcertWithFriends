    ...
    S: Serialize,
{
    let result = auto_reloader
        ...
        .and_then(|env| match env.get_template(template) {
            Ok(template_value) => template_value
                .render(context)
                .or_else(|_| Err(internal_error::<String, minijinja::Error>(None))),
            Err(err) => {
...
fn get_error_500(error: &impl Error, env: &EnvironmentGuard) -> (StatusCode, String) {
    let error_500 = env.get_template("internal_error.j2");
